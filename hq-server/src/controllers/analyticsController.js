/**
 * Analytics Controller — OpenAI-powered insights
 * Facility Admin only.
 *
...
function prescriptiveRules({ avgWaitTime, completionRate, activeQueue,
    todayPatients, peakHour, topService, forecast }) {
  const rules = [];

  if (avgWaitTime > 45)
    rules.push({ severity: 'high', code: 'WAIT_CRITICAL',
    ...
    rules.push({ severity: 'medium', code: 'WAIT_ELEVATED',
  ...
  if (completionRate < 60 && todayPatients > 0)
    rules.push({ severity: 'high', code: 'COMPLETION_LOW',
    ...
    rules.push({ severity: 'medium', code: 'COMPLETION_MODERATE',
  ...
  if (activeQueue >= 15)
    rules.push({ severity: 'high', code: 'QUEUE_OVERLOAD',
    ...
    rules.push({ severity: 'medium', code: 'QUEUE_ELEVATED',
  ...
  if (forecast?.trend === 'increasing' && forecast.slope > 1)
    rules.push({ severity: 'medium', code: 'VOLUME_TRENDING_UP',
  ...
  if (forecast?.trend === 'decreasing' && forecast.slope < -1)
    rules.push({ severity: 'low', code: 'VOLUME_TRENDING_DOWN',
...
async function generateNarrative(metrics, rules, forecast, clinicName, openaiClient) {
  const rulesText = rules.map(r =>
    `[${r.severity.toUpperCase()}] ${r.finding} → ${r.action}`
  ).join('\n');

  const prompt = `You are a healthcare operations analyst for ${clinicName}, a diagnostic clinic in the Philippines.

Based on today's operational data and rule-based analysis, write a concise prescriptive analytics report.

## Today's Metrics
- Patients served today: ${metrics.todayPatients}
- Currently in queue: ${metrics.activeQueue}
- Completed visits: ${metrics.completedToday}
- Average wait time: ${metrics.avgWaitTime} min
...
const getAiInsights = async (req, res) => {
  ...
  if (!OPENAI_KEY) return res.status(503).json({ message: 'OpenAI not configured on server.' });
  ...
  try {
    ...
    const completionRate = todayPatients > 0
      ? Math.round((completedToday / todayPatients) * 100) : 0;
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const c = await QueueEntry.countDocuments({ clinic: cid, joinedAt: { $gte: d, $lte: e } });
      weekSeries.push(c);
    }
    ...
    const topService = svcAgg.length > 0
      ? { name: svcAgg[0]._id, count: svcAgg[0].count } : null;
